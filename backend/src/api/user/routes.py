...
async def edit_user_endpoint(
    ...
    if is_admin is not None:
        if not current_user.is_admin:
            raise HTTPException(status_code=http_status.HTTP_403_FORBIDDEN, detail="Only admins can change admin flag")
        update_kwargs["is_admin"] = is_admin

    if not update_kwargs:
        raise HTTPException(status_code=http_status.HTTP_400_BAD_REQUEST, detail="No changes provided")
